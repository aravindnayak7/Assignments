...
public class ReminderController {
	 ...
	 */
	@GetMapping("/reminder")
	public ResponseEntity<?> getAllReminderhandler(){
		List<Reminder> newsList = remiderservice.getAllReminders();
	...
	public ResponseEntity<?> getReminderByIdHandler(@PathVariable int reminderId) throws ReminderNotExistsException{
		Reminder rm=null;
		try {
			rm = remiderservice.getReminder(reminderId);
			response = new ResponseEntity<>(rm,HttpStatus.OK);
	...
	public ResponseEntity<?> saveReminderHandler(@RequestBody Reminder reminder) throws ReminderNotExistsException{
	
			Reminder found =null; 
			
			try {
				found = remiderservice.getReminder(reminder.getReminderId());
				Reminder res = remiderservice.addReminder(reminder);
	...
	public ResponseEntity<?> upateReminderHandler(@RequestBody Reminder reminder, @PathVariable int reminderId) throws ReminderNotExistsException{
		Reminder upNews = remiderservice.getReminder(reminderId);
		Reminder rm=null;
		try {
			rm= remiderservice.updateReminder(upNews);
			response = new ResponseEntity<>(rm,HttpStatus.OK);
	...
	public ResponseEntity<?> delNewsHandler(@PathVariable int reminderId) throws ReminderNotExistsException{
		try {
		remiderservice.deleteReminder(reminderId);
